...
sudo ssh -o StrictHostKeyChecking=no  -i $masterKops_Key_Loc  ubuntu@$ipAddress -- "kops delete cluster --name=webstar.basic-dev-ops-site-trial.co --state=s3://basic-kops-state --yes"
...
sudo ssh -o StrictHostKeyChecking=no  -i $masterKops_Key_Loc ubuntu@$ipAddress -- "kops create cluster --name=webstar.basic-dev-ops-site-trial.co --state=s3://basic-kops-state --zones=eu-north-1a --node-count=1 --node-size=t3.small --control-plane-zones=eu-north-1a --control-plane-size=t3.medium --dns-zone=webstar.basic-dev-ops-site-trial.co --node-volume-size=8 --control-plane-volume-size=8"
...
sudo ssh -o StrictHostKeyChecking=no  -i $masterKops_Key_Loc ubuntu@$ipAddress -- "kops update cluster --name webstar.basic-dev-ops-site-trial.co --state=s3://basic-kops-state --yes --admin"
