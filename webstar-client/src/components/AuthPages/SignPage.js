...
import ErrorRenderer from "../../components/HelperComponents/ErrorHandler";
import AuthenticPageCss from '../../css/AuthenticPage.module.css'
import StatusHandlerCss from '../../css/StatusHandler.module.css'
import StatusRenderer from "../../components/HelperComponents/StatusHandler";
...
const SignPage = () => {
    ...
    async function onClick() {
        ...
        await axiosConfig({
            url: `/user/${pageType == 'signin' ? 'signin' : 'signup'}`,
            method: 'post',
            data: { email: emailInputRef.current.value, password: passwordInputRef.current.value },
            contentType: "application/json",
        }).then((res) => {
            dispatch(addToAuth({ accessToken: res?.data?.accessToken }))
